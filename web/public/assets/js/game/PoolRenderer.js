...
export class PoolRenderer{
  ...
  async init(){
    ...
    this.scene=new THREE.Scene();this.scene.background=new THREE.Color(0x030706);
  ...
  makeBallTexture(id){const T=this.THREE,size=this.preset==='very-low'?64:this.preset==='normal'?128:256,c=document.createElement('canvas');c.width=c.height=size;const x=c.getContext('2d');x.fillStyle=BALL_COLORS[id];x.fillRect(0,0,size,size);if(id>=9){x.fillStyle='#f7f5ea';x.fillRect(0,0,size,size*.27);x.fillRect(0,size*.73,size,size*.27)}x.beginPath();x.arc(size/2,size/2,45,0,Math.PI*2);x.fillStyle='#f5f2e8';x.fill();x.fillStyle='#101010';x.font='bold 48px system-ui';x.textAlign='center';x.textBaseline='middle';x.fillText(String(id),size/2,size/2+2);const tex=new T.CanvasTexture(c);tex.colorSpace=T.SRGBColorSpace;return tex}
  ...
  setBalls(balls,hard=false){this.ensureBalls(balls);for(const b of balls){this.target.set(b.id,b);const m=this.ballMeshes.get(b.id);if(hard)m.position.set(b.x,b.y,b.z ?? this.table.ball.radius);m.visible=b.state!=='pocketed'&&b.state!=='off_table'}}
  ...
  render(now=performance.now()){
    ...
    for(const [id,t] of this.target){const m=this.ballMeshes.get(id);if(!m)continue;const z=t.z ?? this.table.ball.radius,dx=t.x-m.position.x,dy=t.y-m.position.y,dz=z-m.position.z,dist=Math.hypot(dx,dy,dz),a=dist>.11?1:1-Math.exp(-dt*22);m.position.x+=dx*a;m.position.y+=dy*a;m.position.z+=dz*a;m.rotation.x+=(t.wy||0)*dt;m.rotation.y-=(t.wx||0)*dt;m.rotation.z+=(t.wz||0)*dt;m.visible=t.state!=='pocketed'&&t.state!=='off_table'}
