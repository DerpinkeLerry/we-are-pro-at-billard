...
const BALL_COLORS=['#f5f5f2','#eac530','#1e5dbb','#d44339','#643b92','#e27325','#2e8b57','#8a2431','#111111','#eac530','#1e5dbb','#d44339','#643b92','#e27325','#2e8b57','#8a2431'];

export class PoolRenderer{
  constructor(canvas,tableConfig,preset='normal'){
    this.canvas=canvas;this.table=tableConfig;this.preset=normalizedPreset(preset);this.ballMeshes=new Map();this.ballLabels=new Map();this.target=new Map();this.zoom=1;this.aimAngle=0;this.pointerWorld={x:0,y:0};this.stats={fps:0,frameMs:0};this.lastFrame=performance.now();this.lastPaint=0;this.frames=0;this.fpsTick=this.lastFrame;this.debug=false;this.ready=this.init();
  }
  async init(){
  ...
  buildAim(){const T=this.THREE;this.aimLine=new T.Line(new T.BufferGeometry().setFromPoints([new T.Vector3(),new T.Vector3(1,0,0)]),new T.LineBasicMaterial({color:0x8fffd2,transparent:true,opacity:.65}));this.aimLine.position.z=.04;this.scene.add(this.aimLine)}
  buildCue(){const T=this.THREE,g=GRAPHICS[this.preset];this.cueGroup=new T.Group();const shaft=new T.Mesh(new T.CylinderGeometry(.008,.013,1.25,g.segments),this.material(0xd6b27e,.45));shaft.position.y=-.625;shaft.castShadow=g.shadows;this.cueGroup.add(shaft);const butt=new T.Mesh(new T.CylinderGeometry(.014,.019,.45,g.segments),this.material(0x2e1d13,.4));butt.position.y=-1.45;butt.castShadow=g.shadows;this.cueGroup.add(butt);this.cueGroup.visible=false;this.scene.add(this.cueGroup)}
  makeBallTexture(id){const T=this.THREE,size=this.preset==='very-low'?96:this.preset==='normal'?128:256,c=document.createElement('canvas'),w=size*2;c.width=w;c.height=size;const x=c.getContext('2d');x.fillStyle=id>=9?'#f7f5ea':BALL_COLORS[id];x.fillRect(0,0,w,size);if(id>=9){x.fillStyle=BALL_COLORS[id];x.fillRect(0,size*.31,w,size*.38)}if(id>0){for(const cx of [w*.25,w*.75]){x.beginPath();x.arc(cx,size*.5,size*.22,0,Math.PI*2);x.fillStyle='#fffdf4';x.fill();x.lineWidth=Math.max(2,size*.025);x.strokeStyle='#161616';x.stroke();x.fillStyle='#080808';x.font=`900 ${size*.245}px Arial,sans-serif`;x.textAlign='center';x.textBaseline='middle';x.fillText(String(id),cx,size*.515)}}const tex=new T.CanvasTexture(c);tex.colorSpace=T.SRGBColorSpace;tex.minFilter=T.LinearFilter;tex.magFilter=T.LinearFilter;return tex}
  makeLowBallLabel(id){const T=this.THREE,c=document.createElement('canvas');c.width=c.height=64;const x=c.getContext('2d');x.beginPath();x.arc(32,32,27,0,Math.PI*2);x.fillStyle='#fffdf4';x.fill();x.lineWidth=5;x.strokeStyle='#111';x.stroke();x.fillStyle='#050505';x.font=`900 ${id>9?27:32}px Arial,sans-serif`;x.textAlign='center';x.textBaseline='middle';x.fillText(String(id),32,33);const tex=new T.CanvasTexture(c),mat=new T.SpriteMaterial({map:tex,transparent:true,depthTest:false,depthWrite:false}),sp=new T.Sprite(mat);sp.scale.set(.028,.028,1);sp.renderOrder=20;return sp}
  ensureBalls(balls){const T=this.THREE,g=GRAPHICS[this.preset],r=this.table.ball.radius;for(const b of balls){if(this.ballMeshes.has(b.id))continue;const mat=this.preset==='very-low'?new T.MeshBasicMaterial({map:this.makeBallTexture(b.id)}):new T.MeshStandardMaterial({map:this.makeBallTexture(b.id),roughness:.22,metalness:0});const m=new T.Mesh(new T.SphereGeometry(r,g.segments,Math.max(8,Math.round(g.segments*.65))),mat);m.castShadow=g.shadows;m.receiveShadow=g.shadows;this.scene.add(m);this.ballMeshes.set(b.id,m);if(this.preset==='very-low'&&b.id>0){const label=this.makeLowBallLabel(b.id);this.scene.add(label);this.ballLabels.set(b.id,label)}}}
  setBalls(balls,hard=false){this.ensureBalls(balls);for(const b of balls){this.target.set(b.id,b);const m=this.ballMeshes.get(b.id),label=this.ballLabels.get(b.id),visible=b.state!=='pocketed'&&b.state!=='off_table';if(hard){m.position.set(b.x,b.y,b.z ?? this.table.ball.radius);label?.position.set(b.x,b.y,(b.z ?? this.table.ball.radius)+this.table.ball.radius*.92)}m.visible=visible;if(label)label.visible=visible}}
  setAim(angle,visible=true){this.aimAngle=angle;const cue=this.target.get(0);if(!cue)return;const len=3,a=new this.THREE.Vector3(cue.x,cue.y,.04),b=new this.THREE.Vector3(cue.x+Math.cos(angle)*len,cue.y+Math.sin(angle)*len,.04);this.aimLine.geometry.setFromPoints([a,b]);this.aimLine.visible=visible;this.cueGroup.position.set(cue.x,cue.y,.055);this.cueGroup.rotation.z=angle-Math.PI/2;this.cueGroup.visible=visible}
  ...
  destroy(){this.renderer?.dispose();for(const m of this.ballMeshes.values()){m.geometry?.dispose();m.material?.map?.dispose();m.material?.dispose()}for(const label of this.ballLabels.values()){label.material?.map?.dispose();label.material?.dispose()}this.debugGroup?.traverse?.(o=>{o.geometry?.dispose?.();o.material?.map?.dispose?.();o.material?.dispose?.()})}
}
