...
func TestLoneParticipantRemainsQueuedUntilOpponentArrives(t *testing.T) {
	l := testLobby(t)
	l.participants = map[string]*Participant{
	...
	l.queue = []string{"A"}
	l.selectPlayersIfPossible()
	if l.active != [2]string{} || l.State != Waiting || len(l.queue) != 1 || l.queue[0] != "A" {
		t.Fatalf("lone participant should wait in FIFO queue: active=%v state=%s queue=%v", l.active, l.State, l.queue)
	}
	...
	if l.active != [2]string{"A", "B"} || l.State != Starting || len(l.queue) != 0 {
		t.Fatalf("first two queued participants should become active: active=%v state=%s queue=%v", l.active, l.State, l.queue)
	}
}
